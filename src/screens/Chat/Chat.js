...
const Chat = ({navigation, route}) => {
  ...
  //   const socket = io('http://192.168.1.8:3000/api/'); // Replace with your server URL
  ...
  const sendMessage = async () => {
    ...
    if (input.trim() || file?.fileName) {
      ...
      fetch('http://192.168.1.8:3000/api/chat/send', requestOptions)
  ...
  const replyMessage = async () => {
    ...
    if (input.trim() || file?.fileName) {
      ...
      fetch('http://192.168.1.8:3000/api/chat/reply', requestOptions)
        .then(response => response.text())
        .then(data => {
