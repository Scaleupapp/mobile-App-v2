      ...
      <TouchableOpacity onPress={toggleExpanded} activeOpacity={0.95}>
        ...
        {expanded && (
          <Animated.View style={styles.expandedContent}>
            ...
            {item.memoryTip && (
              <View style={styles.memoryTipSection}>
                <View style={styles.memoryTipHeader}>
                  <MaterialIcons name="psychology" size={18} color={COLORS.purple} />
            ...
            {/* Related Topics - Fixed horizontal scroll */}
...
const LearningVault = () => {
  ...
  const fetchData = async (isRefresh = false, pageNum = 1) => {
    try {
      ...
      setHasMore(pagination && explanations.length + (newExplanations || []).length < pagination.total);
      setPage(pageNum);
    } catch (error) {
      console.error('Error fetching learning vault:', error);
...
💡 Key Insight: ${item.explanation.core}

🎯 Remember: ${item.explanation.keyTakeaway}
    ...
    <View style={styles.header}>
      ...
      <View style={styles.topNavigation}>
        <TouchableOpacity
          ...
          hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
          <Ionicons name="arrow-back" size={24} color={COLORS.blue043142} />
        </TouchableOpacity>
        ...
        <TouchableOpacity 
          ...
          hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
          <MaterialIcons name="analytics" size={24} color={COLORS.blue043142} />
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  decorativeDot: {
    ...
    height: 4,
    borderRadius: 2,
    backgroundColor: COLORS.whiteFFFFFF + '40',
  },
  statsSection: {
    paddingVertical: 16,
    backgroundColor: COLORS.greyF7F7F7,
  ...
  topicsScrollView: {
    marginHorizontal: -16, // Extend scroll area to edges
  },
