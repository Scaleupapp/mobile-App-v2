...
const MyPlaylists = ({route, navigation}) => {
  ...
  const markPostAsViewed = async (playlistId, postId) => {
    try {
      await axios.post(
        'http://192.168.1.8:3000/api/playlists/mark-viewed',
        {
          userId,
          playlistId,
          postId,
    ...
    const fetchUserPlaylists = async () => {
      try {
        ...
        const response = await axios.get(
          `http://192.168.1.8:3000/api/playlists?userId=${userId}`,
          {
            headers: {
              Authorization: `Bearer ${currentToken}`,
            },
  ...
  const handleVideoEnd = async () => {
    ...
    try {
      ...
      if (selectedVideo.isPublic && expandedPublicPlaylist) {
        ...
        const response = await axios.get(
          `http://192.168.1.8:3000/api/playlists/public/${selectedVideo.playlistId}/progress`,
          {
            params: {userId},
            headers: {
  ...
  const togglePlaylistStatus = async playlistId => {
    try {
      const response = await axios.put(
        'http://192.168.1.8:3000/api/playlists/toggle-status',
        {
          userId,
          playlistId,
        },
  ...
  const handleCreatePlaylist = async playlistData => {
    try {
      const response = await axios.post(
        'http://192.168.1.8:3000/api/playlists/create',
        {
          userId,
          ...playlistData,
        },
  ...
  const renamePlaylist = async () => {
    ...
    try {
      const response = await axios.put(
        'http://192.168.1.8:3000/api/playlists/rename',
        {
          userId,
  ...
  const deletePlaylist = async playlistId => {
    try {
      await axios.delete('http://192.168.1.8:3000/api/playlists/delete', {
  ...
  const removePostFromPlaylist = async (playlistId, postId) => {
    try {
      const response = await axios.delete(
        'http://192.168.1.8:3000/api/playlists/remove-from-playlist',
        {
          data: {
            userId,
            playlistId,
  ...
  const savePlaylistOrder = async playlistId => {
    ...
    try {
      await axios.put(
        'http://192.168.1.8:3000/api/playlists/update-order',
        {
          playlistId,
  ...
  const PublicPlaylistItem = ({
    ...
    const fetchPublicPlaylistProgress = async () => {
      ...
      try {
        setError(null);
        const response = await axios.get(
          `http://192.168.1.8:3000/api/playlists/public/${playlist._id}/progress`,
          {
            params: {userId},
            headers: {
              'Content-Type': 'application/json',
