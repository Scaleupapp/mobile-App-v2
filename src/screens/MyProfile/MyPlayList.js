...
const openPublicPlaylistCommentsModal = (playlist) => {
  ...
  const togglePlaylistStatus = async (playlistId) => {
    try {
      ...
      const updatedPlaylists = playlists.map((playlist) =>
        playlist._id === playlistId ? { ...playlist, status: response.data.playlist.status } : playlist
      );
      setPlaylists(updatedPlaylists);
    } catch (error) {
      console.error('Failed to toggle playlist status:', error);
      Alert.alert('Error', error.response?.data?.message || 'Failed to toggle playlist status');
    }
  };
  ...
  const fetchPublicPlaylists = async () => {
    try {
      ...
      Alert.alert('Error', error.response?.data?.message || 'Failed to fetch public playlists');
    }
  };
    ...
    try {
      ...
      const postDetailsPromises = response.data.items.map(async (item) => {
        try {
          ...
          console.error(`Failed to fetch details for post ${item.postId}:`, err);
          return null;
        }
      });
      ...
      Alert.alert('Error', error.response?.data?.message || 'Failed to fetch playlist details');
    }
  };
            ...
            {playlists.map((playlist) => (
              <View key={playlist._id} style={styles.editPlaylistItem}>
                <Text variant="medium14" color={COLORS.blue043142} style={{ flex: 1 }}>
                  {playlist.playlistName} ({playlist.status}) {/* Display status */}
                </Text>
                <TouchableOpacity
                  onPress={() => {
                ...
                >
                  <Icon
                    type="ionicon"
                    name={playlist.status === 'private' ? 'lock-closed' : 'earth'}
                    size={20}
  ...
  const renderPlaylistItem = (playlist) => {
    ...
    const viewedPosts = playlist.items.filter(item => 
      item.viewedStatus?.find(status => 
        status.userId === userId && status.isViewed
      )
    ).length;
              ...
              return (
                <Swipeable
                  ...
                  renderRightActions={(progress, dragX) => 
                    renderRightActions(progress, dragX, item, index, playlist._id)
                  }
                >
                  <View style={styles.videoItemContainer}>
                    ...
                    <View style={styles.actionButtons}>
                      ...
                      >
                        <Icon type="ionicon" name="trash" size={24} color={COLORS.red} />
                      </TouchableOpacity>
                      {isViewed && (
                        <Icon 
                          type="ionicon" 
    ...
    return (
      <View style={styles.playlistContainer}>
        ...
        >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <TouchableOpacity onPress={() => openPublicPlaylistCommentsModal(playlist)}>
            <Icon 
              type="ionicon" 
              name="chatbubble-ellipses-outline" 
              size={24} 
...
{isPublicPlaylistsExpanded && publicPlaylists.length > 0 ? (
  <View >
    {publicPlaylists.map(renderPublicPlaylistItem)}
  </View>
