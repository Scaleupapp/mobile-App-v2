...
const PlaylistCommentsModal = ({
  ...
  const checkPlaylistOwnership = async () => {
    try {
      const response = await axios.get(
        `http://192.168.68.240:3000/api/playlists/${playlistId}/ownership`,
        {
          params: {userId},
        },
      );
  ...
  const pinComment = async commentId => {
    try {
      setIsLoading(true);
      await axios.post(
        'http://192.168.68.240:3000/api/playlists/comments/pin',
        {
          userId,
          playlistId,
          commentId,
  ...
  const fetchComments = async () => {
    ...
    try {
      const response = await axios.get(
        `http://192.168.68.240:3000/api/playlists/${playlistId}/comments`,
      );
  ...
  const addComment = async () => {
    ...
    try {
      setIsLoading(true);
      await axios.post(
        'http://192.168.68.240:3000/api/playlists/comments/add',
        {
          userId,
          playlistId,
          text: newCommentText,
  ...
  const replyToComment = async commentId => {
    ...
    try {
      setIsLoading(true);
      await axios.post(
        'http://192.168.68.240:3000/api/playlists/comments/reply',
        {
          userId,
          playlistId,
          commentId,
  ...
  const likeComment = async (commentId, replyId = null) => {
    try {
      setIsLoading(true);
      await axios.post(
        'http://192.168.68.240:3000/api/playlists/comments/like',
        {
          userId,
          playlistId,
          commentId,
