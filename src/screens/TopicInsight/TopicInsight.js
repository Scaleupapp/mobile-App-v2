...
// Practice Question Component - Fixed Version
...
const additionalStyles = {
  ...
  const getOptionColor = (option) => {
    ...
    const correctLetter = question.correctAnswer.trim();
    if (optionLetter === correctLetter) return COLORS.greenSuccess + '15';
    if (option === selectedAnswer && optionLetter !== correctLetter) return COLORS.redError + '15';
  ...
  const getOptionBorderColor = (option) => {
    ...
    const optionLetter = option.split(')')[0].trim();
    const correctLetter = question.correctAnswer.trim();
    if (optionLetter === correctLetter) return COLORS.greenSuccess;
    if (option === selectedAnswer && optionLetter !== correctLetter) return COLORS.redError;
    return COLORS.greyD6D6D6;
  };

  // Check if the selected answer is correct with proper comparison
  const isCorrectAnswer = selectedAnswer && selectedAnswer.split(')')[0].trim() === question.correctAnswer.trim();
    ...
    <View style={styles.practiceQuestion}>
      ...
      <View style={styles.optionsContainer}>
        {question.options.map((option, idx) => (
          <TouchableOpacity
            ...
            <Text 
              variant="regular13" 
              color={showExplanation && option.split(')')[0].trim() === question.correctAnswer.trim() ? COLORS.greenSuccess : COLORS.grey333333}>
              {option}
            </Text>
            {showExplanation && option.split(')')[0].trim() === question.correctAnswer.trim() && (
              <MaterialIcons name="check-circle" size={18} color={COLORS.greenSuccess} />
            )}
            {showExplanation && option === selectedAnswer && option.split(')')[0].trim() !== question.correctAnswer.trim() && (
      ...
      {showExplanation && (
        <View style={[
          ...
          {
            backgroundColor: isCorrectAnswer ? COLORS.greenSuccess + '08' : COLORS.redError + '08',
            borderLeftWidth: 3,
            borderLeftColor: isCorrectAnswer ? COLORS.greenSuccess : COLORS.redError,
          }
        ]}>
          <View style={styles.explanationHeader}>
            <MaterialIcons 
              ...
              size={16} 
              color={isCorrectAnswer ? COLORS.greenSuccess : COLORS.redError} 
            />
            <Text 
              variant="semibold13" 
              color={isCorrectAnswer ? COLORS.greenSuccess : COLORS.redError}
              style={styles.explanationStatus}>
