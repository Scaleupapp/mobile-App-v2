...
import { getHomePageData, getRecommendedContent, getProfile } from '../../services/apiService';
...
const processContentList = (contentList, isRecommended = false) => {
  return contentList.map((item) =>
    isRecommended ? transformRecommendedContent(item) : transformNormalContent(item)
  );
};
...
const Home = ({ navigation, route }) => {
  ...
  const ListHeader = () => (
    <View style={styles.listHeaderContainer}>
      ...
      <Text variant="semibold12" style={styles.feedHeading} color={COLORS.blue043142}>
